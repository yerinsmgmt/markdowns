...
- **FACE** you are on camera. The words are written out. Say them your way.
- **READ** you are not on camera. Read it exactly as written. Nobody sees your
  mouth, so you cannot fumble it.
- **SCREEN** screen recording. Recorded clean after the build, never live.
- **GFX** motion graphic. Nothing for you to do. It plays over a READ block.

## How to record it

**Record everything as video, including the READ blocks.** Do not set up a
separate audio workflow. Point the camera at the wall, at your desk, at
anything, and read. I strip the video off and keep the sound. One workflow, one
device, nothing to configure.

**Do not stop and start between blocks.** Record one continuous take per lesson
per kind. So lesson 2 gives you three files:

```
L2-FACE.mov      every FACE block in lesson 2, one take
L2-READ.mov      every READ block in lesson 2, one take
L2-SCREEN-01.mov  each screen recording separately, they are separate captures
L2-SCREEN-02.mov
```

**Say the block name out loud before you say the line.** Just "face one",
pause, then the line. Pause again, "face two", and so on. That is the only
thing I need from you, and it is what lets me cut a forty minute take into
twenty clips without asking you anything.

**If you fluff a line, do not start the file again.** Pause, say the block name
again, and redo just that line. I take the last good version of each. That is
the whole point of one long take.

**Format:** whatever your camera gives you. MOV, MP4, anything. Do not convert
it, do not compress it, do not trim it. Send it raw.
...
### GFX-01
Title card. "Build real software with AI." Over L1-READ-01.
